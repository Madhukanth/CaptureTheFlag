...
import { pointsChanged, question18Changed } from '../actions/pointsChange';
import axios from 'axios';
import { Alert } from 'react-bootstrap';

const str =
	'18.To reach next level you must know the  ASCII code <-> letter conversion. Clue(97 - a, 65 – A) ';
const str1 = '<script language="Javascript">';
const str2 = 'moo = unescape(`%m%j%q%c%e%w%h`);';
...
class Eighteen extends Component {
	...
	update = () => {
		if (
			this.state.value === '10910611399101119104' &&
			this.props.question18 === true
		) {
			this.props.question18Changed();
...
const mapStateToProps = state => ({
	...
	question18: state.points.question18
});

export default connect(mapStateToProps, { pointsChanged, question18Changed })(
	withRouter(Eighteen)
);
