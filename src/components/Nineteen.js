...
import { pointsChanged, question19Changed } from '../actions/pointsChange';
import axios from 'axios';
import { Alert } from 'react-bootstrap';

const str =
  '19. To reach next level you must know the  ASCII code <-> letter conversion. Clue(97 - a, 65 – A)';
const str1 = '<script language="Javascript">';
const str2 = 'moo = unescape(`%A%H%M%D%O%W%L%Z`);';
...
class Nineteen extends Component {
  ...
  update = () => {
    if (
      this.state.value === '65727768797690' &&
      this.props.question19 === true
    ) {
      this.props.question19Changed();
...
const mapStateToProps = state => ({
  ...
  question19: state.points.question19,
});

export default connect(mapStateToProps, { pointsChanged, question19Changed })(
  withRouter(Nineteen),
);
